...
/** Split narration into subtitle beats (sentence-ish), proportional-timed later. */
function splitCaptions(text: string, maxBeats = 4): string[] {
  const parts = text
    .split(/(?<=[.!?])\s+/)
    .map((s) => s.trim())
    .filter(Boolean);
  if (parts.length <= maxBeats) return parts;
  const per = Math.ceil(parts.length / maxBeats);
  const groups: string[] = [];
...
export async function renderVideoHyperframes(
  ...
  // 2) Synthesize narration, measure, lay scenes out, and collect subtitle cues.
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const voStart = cursor + intro;
    scenes.push({ ...p.scene, start: cursor, dur, voStart, voDur, audio: audioRel });

    // Subtitle cues: split the narration into beats, timed across the VO window
    // proportional to length. These become a soft (toggleable) track, not pixels.
    if (p.voText.trim() && voDur > 0) {
      const beats = splitCaptions(p.voText, 6);
      const totalW = beats.reduce((a, b) => a + Math.max(8, b.length), 0);
      let cum = 0;
      for (const b of beats) {
        const cs = voStart + (voDur * cum) / totalW;
        cum += Math.max(8, b.length);
        const ce = voStart + (voDur * cum) / totalW;
        cues.push({ start: cs, end: ce, text: b });
      }
    }
    cursor += dur;
  }
  ...
  // Render to a temp file when we'll mux subtitles in afterwards.
  const renderTarget = cues.length ? join(projDir, "render.mp4") : outFile;
  const r = spawnSync(
    "npx",
    ["--yes", `hyperframes@${HYPERFRAMES_VERSION}`, "render", projDir, "-o", renderTarget, "-q", "high", "-f", String(fps)],
    { encoding: "utf8", env, maxBuffer: 64 * 1024 * 1024, stdio: ["ignore", "pipe", "pipe"] },
  );
  if (r.status !== 0 || !existsSync(renderTarget)) {
  ...
  // Mux the narration in as a soft (toggleable) subtitle track, plus a sidecar
  // .srt. forced=0 so it's never burned into the frame. (MP4/mov_text always
  // flags a lone subtitle default=1 — an ffmpeg movenc limitation we can't clear
  // here — but players still gate display on the CC toggle and render it via the
  // player's own subtitle engine, not as baked pixels.)
  if (cues.length) {
    const srt = buildSrt(cues);
    const srtPath = join(projDir, "captions.srt");
    writeFileSync(srtPath, srt);
    const mux = spawnSync(
      ffmpeg,
      [
        "-y", "-i", renderTarget, "-i", srtPath,
        "-map", "0", "-map", "1",
        "-c", "copy", "-c:s", "mov_text",
        "-disposition:s:0", "0",
    ...
    if (mux.status !== 0 || !existsSync(outFile)) {
      throw new Error(`subtitle mux failed:\n${(mux.stderr ?? "").split("\n").slice(-6).join("\n")}`);
    }
    writeFileSync(outFile.replace(/\.mp4$/i, "") + ".srt", srt);
  }
